...
logging.basicConfig(
    format="[%(asctime)s] %(levelname)s \
        [%(module)s.%(funcName)s:%(lineno)d] %(message)s", datefmt="%H:%M:%S"
)
...
# Validates role pathname whitelist as passed via AWS Config
# parameters and returns a list of comma separated patterns.
def validate_whitelist(unvalidated_role_pattern_whitelist):
    # Names of users, groups, roles must be alphanumeric,
    # including the following common
    # characters: plus (+), equal (=), comma (,), period (.),
...
# This uses Unix filename pattern matching (as opposed to regular
# expressions), as documented here:
# https://docs.python.org/3.7/library/fnmatch.html.
# Please note that if using a wildcard, e.g. "*", you should use
...
def is_whitelisted_role(role_pathname, pattern_list):
    ...
    # eg. /service-role/aws-codestar-service-role matches
...
# Form an evaluation as a dictionary. Suited to report on scheduled rules.
# More info here:
# https://boto3.amazonaws.com/v1/documentation/api/latest/reference
# /services/config.html#ConfigService.Client.put_evaluations
...
def determine_last_used(
    ...
    days_unused = (
        ...
        last_used_date.replace(tzinfo=None)).days

    if days_unused > max_age_in_days:
        compliance_result = NON_COMPLIANT
        reason = f"Was used {days_unused} days ago in {used_region}"
        logger.info(f"NON_COMPLIANT: {role_name} has not been used for \
            {days_unused} days, last use in {used_region}")
        return build_evaluation(
            role_name,
            compliance_result,
            notification_creation_time,
    ...
    reason = f"Was used {days_unused} days ago in {used_region}"
    logger.info(f"COMPLIANT: {role_name} used \
        {days_unused} days ago in {used_region}")
    return build_evaluation(
        role_name,
        compliance_result,
        notification_creation_time,
...
# Returns a list of docs, each of which has authorization
# details of each role.  More info here:
#   https://boto3.amazonaws.com/v1/documentation/api/latest/reference/
#   services/iam.html#IAM.Client.get_account_authorization_details
def get_role_authorization_details(iam_client):
    ...
    while True:
        ...
        if 'Marker' in roles_list:
            roles_list = iam_client.get_account_authorization_details(
                Filter=['Role'],
                MaxItems=100,
                Marker=roles_list['Marker'])
        else:
            break
...
# Check the compliance of each role by determining if role
# last used is > than max_days_for_last_used
def lambda_handler(event, context):
    ...
    notification_creation_time =\
        str(json.loads(event['invokingEvent'])['notificationCreationTime'])

    # ruleParameters is received from AWS Config's user-defined parameters
    rule_parameters = json.loads(event["ruleParameters"])

    # Maximum allowed days that a role can be unused,
    # or has been last used for an AWS request
    max_days_for_last_used =\
        int(os.environ.get('max_days_for_last_used', MAX_UNUSED_DAYS))
    ...
    if 'role_whitelist' in rule_parameters:
        whitelisted_role_pattern_list =\
            validate_whitelist(rule_parameters['role_whitelist'])

    # Iterate over all our roles.  If the creation date of a
    # role is <= max_days_for_last_used, it is compliant
    for role in all_roles:
        ...
        role_age_in_days = (
            datetime.datetime.now() -
            role_creation_date.replace(tzinfo=None)).days
        ...
        if role_age_in_days <= max_days_for_last_used:
            ...
            evaluations.append(
                build_evaluation(
                    ...
                    resource_type=DEFAULT_RESOURCE_TYPE,
                    annotation=reason))
            logger.info(f"COMPLIANT: {role_name} - {role_age_in_days} \
                is newer or equal to {max_days_for_last_used} days")
            continue
    ...
    # Iterate over our evaluations 100 at a time,
    # as put_evaluations only accepts a max of 100 evals.
